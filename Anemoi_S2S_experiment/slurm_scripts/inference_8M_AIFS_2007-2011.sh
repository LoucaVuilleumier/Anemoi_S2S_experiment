...
# The job name
#SBATCH --job-name=inference_8M_AIFS_2007-20011
...
# Automatically generate all first, the 11th and the 21st from 2007-2011
...
for year in {2007..2011}; do
    for month in {5..9}; do
        ...
        OUTPUT_DIR="/ec/res4/scratch/nld4584/Anemoi_S2S_experiment/output_inference/AIFS/aifs-subs-pretrain-${first_day}"
        mkdir -p ${OUTPUT_DIR}
        ...
        OUTPUT_DIR="/ec/res4/scratch/nld4584/Anemoi_S2S_experiment/output_inference/AIFS/aifs-subs-pretrain-${eleventh}"
        mkdir -p ${OUTPUT_DIR}
        OUTPUT_FILE="${OUTPUT_DIR}/member-${SLURM_ARRAY_TASK_ID}.nc"
        export INFERENCE_OUTPUT_PATH=${OUTPUT_FILE}
        export INFERENCE_DATE=${BASE_DATE}
        ...
        OUTPUT_DIR="/ec/res4/scratch/nld4584/Anemoi_S2S_experiment/output_inference/AIFS/aifs-subs-pretrain-${twenty_first}"
        mkdir -p ${OUTPUT_DIR}
        OUTPUT_FILE="${OUTPUT_DIR}/member-${SLURM_ARRAY_TASK_ID}.nc"
        export INFERENCE_OUTPUT_PATH=${OUTPUT_FILE}
        export INFERENCE_DATE=${BASE_DATE}
