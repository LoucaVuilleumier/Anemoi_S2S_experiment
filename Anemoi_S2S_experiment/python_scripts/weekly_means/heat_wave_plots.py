...
OUTPUT_DIR = "/ec/res4/hpcperm/nld4584/Anemoi_S2S_experiment/output_metrics/AIFS-75-init/heat_wave_final_plots"
...
def plot_ensemble_plumes(var_name, datasets_dict, init_date, region_mask, output_dir, best_members=None):
    ...
    model_colors = {
        ...
        'daily_finetuned_ds': '#2ca02c'    # green
    }
...
def plot_ensemble_plumes_anomalies(var_name, anomalies_dict, init_date, region_mask, output_dir, best_members=None):
    ...
    model_colors = {
        ...
        'weekly_finetuned_anomalies_ds': '#d62728', # red
        'daily_finetuned_anomalies_ds': '#2ca02c'    # green
    }
...
def plot_probabilistic_metrics(list_ds_hw_metrics_aggregated, list_reliability_data, list_model_names, output_dir):
    ...
    for idx, severity in enumerate(severities):
        ...
        ax.set_ylabel('Brier Skill Score', fontsize=11, fontweight='bold')
        ax.set_title(f'{severity_labels[severity]} Heat Waves', fontsize=12, fontweight='bold')
        ax.grid(True, alpha=0.3)
    ...
    for severity in ['moderate', 'severe', 'extreme']:
        ...
        for idx, week in enumerate(weeks_sorted):
            ...
            ax.axhline(clim, color='gray', linestyle=':', linewidth=1, alpha=0.5, 
            ...
            ax.set_title(f'Week {week_num} Lead', fontsize=11, fontweight='bold')
            ax.grid(True, alpha=0.3)
            
            # Add legend only to first subplot
            if idx == 0:
                ax.legend(fontsize=8, loc='upper left', frameon=True, framealpha=0.9, markerscale=0.3)
    ...
    for severity in severities:
        ...
        for model_idx in range(3):
            if row_cf[model_idx] is not None:
                ...
                cbar.set_label('BSS', fontsize=10, fontweight='bold')
        ...
        for model_idx, model_name in enumerate(list_model_names):
            ...
            fig.text(0.01, y_center, model_name, fontsize=13, fontweight='bold', 
...
def plot_economic_value(economic_value_data):
    """
    Plot economic value curves for different lead times
    Creates 3 plots (one per severity) with 4 curves each (Week 1, 2, 4, 8)
    """
    print("\nCreating economic value plots...")
    ...
    for severity in ['moderate', 'severe', 'extreme']:
        ...
        data_severity = economic_value_data[severity]
        
        for idx, week_lead in enumerate(lead_weeks):
            # Find data for this lead time
            data_lead = [d for d in data_severity if d['week_lead'] == week_lead][0]
            
            cl_ratios = data_lead['cl_ratios']
            values = data_lead['values']
            clim_freq = data_lead['clim_freq']
            
            # Plot value curve
            ax.plot(cl_ratios, values, linewidth=2.5, color=colors[idx], 
                   label=f'Week {week_lead}', marker='o', markersize=3, markevery=10)
        
        # Add reference lines
        ax.axhline(0, color='black', linestyle='--', linewidth=1, alpha=0.5, label='No value')
        
        # Mark climatology frequency on x-axis (where forecast typically offers greatest benefit)
        clim_freq_mean = np.mean([d['clim_freq'] for d in data_severity[:4]])  # Average over plotted weeks
        ax.axvline(clim_freq_mean, color='gray', linestyle=':', linewidth=1.5, alpha=0.5, 
                  label=f'Climatology (C/L ≈ {clim_freq_mean:.2f})')
        
        ax.set_xlabel('Cost-Loss Ratio (C/L)', fontsize=13, fontweight='bold')
        ax.set_ylabel('Potential Economic Value', fontsize=13, fontweight='bold')
        ax.set_title(f'Economic Value: {severity.capitalize()} Heat Wave Forecasts over Europe',
                    fontsize=14, fontweight='bold')
        ax.set_xlim([0, 1])
        ax.set_ylim([0, 1])
        ...
        # Add text annotation
        ax.text(0.05, 0.95, 'Higher values = greater forecast benefit\nC/L < climatology freq: always beneficial',
               transform=ax.transAxes, fontsize=10, verticalalignment='top',
               bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.3))
        
        plt.tight_layout()
        
        output_file = os.path.join(OUTPUT_DIR, f'economic_value_{severity}.png')
        plt.savefig(output_file, dpi=200, bbox_inches='tight')
        print(f"  Saved: {output_file}")
        plt.close()
...
best_members = find_best_members(datasets_dict, anomalies_dict, init_date, region_mask)

# Map best members from anomaly dataset names to raw dataset names
best_members_raw = {
    'ref_weekly_ds': best_members.get('ref_anomalies_ds'),
    'weekly_finetuned_ds': best_members.get('weekly_finetuned_anomalies_ds'),
    'daily_finetuned_ds': best_members.get('daily_finetuned_anomalies_ds')
}
...
plot_probabilistic_metrics(list_ds_hw_metrics_aggregated, list_reliability_data, list_model_names, OUTPUT_DIR)
print("\n" + "="*80)
...
plot_bss_spatial_maps(list_ds_hw_metrics_spatial, list_model_names, europe_mask, lat, lon, OUTPUT_DIR)
print("\n" + "="*80)
print("All probabilistic spatial metric plots completed!")
print(f"Output directory: {OUTPUT_DIR}")

# ============================================================================
# ECONOMIC VALUE PLOTS
# ============================================================================

plot_economic_value(economic_value_data)